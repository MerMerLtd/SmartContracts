...
contract XPAExchange is SafeMath, Authorization, StandardToken {
    ...
    address public XPAToken = address(this);
    ...
    /* user data */
    mapping(address => mapping(address => uint256)) public balanceSheet;
    ...
    */
    ...
    function checkBalance(
        ...
        internal
    returns(bool) {
        if(safeAdd(balanceSheet[user_][token_], depositAmount_) >= amount_) {
            return true;
        } else {
            emit Error(0);
            return false;
    ...
    returns(bool) {
        if(addOrSub_) {
            balanceSheet[user_][token_] = safeAdd(balanceSheet[user_][token_], amount_);
        } else {
            if(checkBalance(user_, token_, amount_, 0)){
                balanceSheet[user_][token_] = safeSub(balanceSheet[user_][token_], amount_);
                return true;
            } else {
                return false;
            }
    ...
    function feeback(
        ...
        external
    returns(bool success) {
        uint256 gasUsed = safeSub(gasLimit, gasleft());
        uint256 fee = safeMul(gasUsed, tx.gasprice);
        mint(msg.sender, fee);
        return true;
    }
